#!/usr/bin/env python3
"""
Gambling category implementation for domain blocklist.
"""
...
class GamblingCategory(BaseCategory):
    """Gambling domain blocklist category"""
    
    def __init__(self):
        super().__init__("gambling")
    
    def get_readme_content(self, stats):
        """Generate README content for gambling category"""
        ...
        readme_content = f"""# 🚫 {stats.get('name', 'Gambling')} Domain Blocklist

{stats.get('description', 'Gambling domain blocklist')}
...
- [`{stats.get('output_file', 'gambling.txt')}`]({stats.get('output_file', 'gambling.txt')}) - Main blocklist file (cumulative)
...
```
https://raw.githubusercontent.com/your-repo/main/blocklist/{stats.get('output_file', 'gambling.txt')}
```

### AdGuard Home
Add as a custom filter:
```
https://raw.githubusercontent.com/your-repo/main/blocklist/{stats.get('output_file', 'gambling.txt')}
```
