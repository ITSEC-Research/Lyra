#!/usr/bin/env python3
"""
Suspicious category implementation for domain blocklist.
"""
...
class SuspiciousCategory(BaseCategory):
    """Suspicious domain blocklist category"""
    
    def __init__(self):
        super().__init__("suspicious")
    
    def get_readme_content(self, stats):
        """Generate README content for suspicious category"""
        ...
        readme_content = f"""# 🚫 {stats.get('name', 'Suspicious')} Domain Blocklist

{stats.get('description', 'Suspicious domain blocklist')}
...
- [`{stats.get('output_file', 'suspicious.txt')}`]({stats.get('output_file', 'suspicious.txt')}) - Main blocklist file (cumulative)
